...
in the computer science literature as a ``container'' \cite{Abbott}, but since such an indexed
...
\begin{align*}
  \term{ZC}(D) &\sim \mbox{the coefficient of $0^X$ in $D$.}\\
  !^{C} &\sim X \mapsto C \times 0^X \\
  \cod(D) &\sim D(0) \\
  \const(C) &\sim X \mapsto C \\
  \dom(D) &\sim D(1) \\
  !_{C} &\sim X \mapsto C^X 
\end{align*}
\dis{If we're using \cref{thm:dirichlet.set.characterization}, shouldn't we be thinking in terms of bundles?} In particular, $!_{(-)}$ corresponds to the Yoneda embedding.
\end{rmk}
...
lex localization (``inverse image functor'') given by the vertical / cartesian factorization system on $\Set^{\down}$.

\begin{thm}
  For any $\pi : \down \to \Set$, we have a subtopos inclusion
  $$\Set^{\down}_{\ulcorner/ \pi} \hookrightarrow \Set^{\down}_{/\pi}$$
  with lex localization given by vertical-cartesian factorization:
  \[
\begin{tikzcd}
